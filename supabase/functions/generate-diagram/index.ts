...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert at creating educational diagrams. Generate a ${type} diagram about ${topic} at ${difficulty} difficulty level. Return the diagram in a structured format with nodes, connections, and descriptions.`;
...
Format the response as a JSON object with 'title', 'description', 'elements' (array), and 'connections' (array).`;
